        ...
        if (@antes > 1) {
            ...
            if (!defined $anode) {
                if ($tnode->t_lemma =~ /^#PersPron/) {
                    push @types, "COREF_TEXT_PRON_DROP";
                }
                else {
                    push @types, "COREF_TEXT_NOM_DROP";
                }
            }
            else {
                # English
                if ($anode->language eq "en") {
                    if ($anode->tag =~ /^(P|DT)/) {
                        push @types, "COREF_TEXT_PRON_OVERT";
                    }
                    else {
                        push @types, "COREF_TEXT_NOM_OVERT";
                    }
                }
                # Czech, Russian
                else {
...
sub process_tnode {
    ...
    print join "\t", ($tnode->get_document->full_filename, $tnode->id, $tnode->language, $tnode->t_lemma, $types, $tnode_entity);
    print "\n";
}
